...
Date.prototype.offsetDay = function(day){
	var t = this;
	return new Date(t.getFullYear(),t.getMonth(),(t.getDate() + (day || 0)));
};
...
String.prototype.toPinyin =function(splitor){
	...
	var PYMapping = {'运':'yun','营':'ying','统':'tong','计':'ji','负':'fu','载':'zai','用':'yong','户':'hu','登':'deng','录':'lu','联':'lian|lain','通':'tong','分':'fen','公':'gong','司':'si','客':'ke','经':'jing','理':'li','出':'chu','账':'zhang|zang','单':'dan','在':'zai|zhai','线':'xian','查':'cha|ca','询':'xun','员':'yuan','工':'gong','操':'cao','作':'zuo','日':'ri','志':'zhi','短':'duan','信':'xin','发':'fa','送':'song','上':'shang','网':'wang','第':'di','三':'san','方':'fang','支':'zhi','付':'fu','订':'ding','购':'gou','记':'ji','卡':'ka','密':'mi','充':'chong|cong','值':'zhi','系':'xi','参':'can','数':'shu|su','配':'pei','置':'zhi','访':'fang','问':'wen','策':'ce','略':'lue','鉴':'jian','权':'quan','授':'shou|sou','时':'shi|si','段':'duan','开':'kai','模':'mu|mo','板':'ban','商':'shang','品':'pin','定':'ding','任':'ren','务':'wu','布':'bu','审':'shen|sen','核':'he','接':'jie','入':'ru','企':'qi','业':'ye','息':'xi','管':'guan','受':'shou|sou','批':'pi','量':'liang','积':'ji','兑':'dui','换':'huan','导':'dao','制':'zhi|zi','广':'guang','告':'gao','规':'gui','则':'ze','位':'wei','页':'ye','面':'mian','监':'jian','控':'kong','警':'jing','类':'lei','型':'xing','义':'yi','知':'zhi','号':'hao','与':'yu','限':'xian','报':'bao','表':'biao','据':'ju','维':'wei','护':'hu','元':'yuan','设':'she','备':'bei','行':'hang','角':'jue|jiao','色':'se|she','组':'zu','织':'zhi|ji','地':'di','域':'yu','厂':'chang|cang','学':'xue','校':'xiao'};
	 var ret = [""];
     for (var i = 0; i < this.length; i++) {
         var c = this.charAt(i);
         var pinyins = (PYMapping[c] || c).toLowerCase().split('|');
         var newRet = [];
...
if(ua.contains("msie") &&( isBelowIE8 =  (ua.match(/msie ([\d.]+)/)[1] < 8))){
    document.write('<script src="/portal/js/json3.min.js"></script>');
}
if(ua.contains("msie") &&( (ua.match(/msie ([\d.]+)/)[1] <= 9))){
    document.write('<script src="/portal/js/placeholders.js"></script>');
}
if(typeof xjl == "undefined") xjl = {};
